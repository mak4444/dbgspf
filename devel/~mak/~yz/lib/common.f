\ 1.04 Часто требующиеся определения, asciiz-строки
...
: LOWORD ( n --n) 0x0000FFFF AND ;
: HIWORD ( n--n)  0xFFFF0000 AND 16 RSHIFT ;
: ON  ( a--) TRUE SWAP ! ;
: OFF ( a--) 0! ;
: ?  @ . ;
: 1-!  -1 SWAP +! ;
...
: escape ( c--c ) CASE 
  ...
  DUP c: 0 c: 9 WITHIN IF
    c: 0 -
    BEGIN ( n) mchar DUP c: 0 c: 9 WITHIN WHILE
      ( n c) c: 0 - SWAP 10 * +
